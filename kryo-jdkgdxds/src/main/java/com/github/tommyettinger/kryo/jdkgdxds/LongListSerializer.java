...
public class LongListSerializer extends Serializer<LongList> {
    ...
    @Override
    public void write(final Kryo kryo, final Output output, final LongList data) {
    ...
    @Override
    public LongList read(final Kryo kryo, final Input input, final Class<? extends LongList> dataClass) {
        int length = input.readInt(true);
        LongList data = new LongList(length);
        for (int i = 0; i < length; i++)
