...
public class EnumIntOrderedMapSerializer extends Serializer<EnumIntOrderedMap> {
    ...
    @Override
    public EnumIntOrderedMap copy(Kryo kryo, EnumIntOrderedMap original) {
        EnumIntOrderedMap map = new EnumIntOrderedMap(original);
        kryo.reference(map);
        map.clear();
        for(EnumIntOrderedMap.Entry ent : original) {
