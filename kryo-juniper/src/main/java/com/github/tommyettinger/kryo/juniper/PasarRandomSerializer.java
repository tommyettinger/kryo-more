...
/**
 * Kryo {@link Serializer} for juniper {@link PasarRandom}s.
 */
public class PasarRandomSerializer extends Serializer<PasarRandom> {
    ...
    @Override
    public void write(final Kryo kryo, final Output output, final PasarRandom data) {
    ...
    @Override
    public PasarRandom read(final Kryo kryo, final Input input, final Class<? extends PasarRandom> dataClass) {
        return new PasarRandom(input.readVarLong(false), input.readVarLong(false),
                input.readVarLong(false), input.readVarLong(false), input.readVarLong(false));
    }

    @Override
    public PasarRandom copy(Kryo kryo, PasarRandom original) {
