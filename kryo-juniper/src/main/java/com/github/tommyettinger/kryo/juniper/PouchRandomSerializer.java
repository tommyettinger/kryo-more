...
/**
 * Kryo {@link Serializer} for juniper {@link PouchRandom}s.
 */
public class PouchRandomSerializer extends Serializer<PouchRandom> {
    ...
    @Override
    public void write(final Kryo kryo, final Output output, final PouchRandom data) {
    ...
    @Override
    public PouchRandom read(final Kryo kryo, final Input input, final Class<? extends PouchRandom> dataClass) {
        return new PouchRandom(input.readVarLong(false), input.readVarLong(false),
                input.readVarLong(false), input.readVarLong(false));
    }

    @Override
    public PouchRandom copy(Kryo kryo, PouchRandom original) {
