...
public class RayleighDistributionSerializer extends Serializer<RayleighDistribution> {
    ...
    @Override
    public RayleighDistribution copy(Kryo kryo, RayleighDistribution original) {
        return original.copy();
    }

    @Override
    public void write(Kryo kryo, Output output, RayleighDistribution object) {
        kryo.writeClassAndObject(output, object.generator);
        output.writeDouble(object.getParameterA());
    }

    @Override
    public RayleighDistribution read(Kryo kryo, Input input, Class<? extends RayleighDistribution> type) {
        return new RayleighDistribution((EnhancedRandom) kryo.readClassAndObject(input), input.readDouble());
    }
}
